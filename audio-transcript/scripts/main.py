...
def run_ml(uuid: str) -> dict:
    try:
        media_service.split_source(uuid=uuid)
        return transcription_service.generate_subs(uuid=uuid)
...
def callback(ch, method, properties, body, threads: list, requests: list):
    try:
